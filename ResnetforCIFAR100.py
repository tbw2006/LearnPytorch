...
#先进行数据预处理，在建立dataset，dataloader，在写残差块，再写model堆叠残差块，再训练
...
train_set = datasets.CIFAR100(
    root='I:\Data\CIFAR100' ,           #改成你要下载数据集的路径
    train= True,
    transform= train_transform,
    download=True)

test_set = datasets.CIFAR100(
    root='I:\Data\CIFAR100',            #这里也一样
    train=False,
        ...
        if inc != ouc or stride!=1:
            self.shortcut = nn.Sequential(
                nn.Conv2d(
                    in_channels=inc,
                    out_channels=ouc,
                    kernel_size=1,    #不需要方括号
                    stride=stride,
                    padding=0),  # padding应为0
                nn.BatchNorm2d(ouc)
            )
...
class custom_resnet(nn.Module):
    def __init__(self, classes = 100):
        ...
        self.layer1 = self._make_layer_(64, 64, num=3, stride=1)   # 不压缩尺寸
        self.layer2 = self._make_layer_(64, 128, num=4, stride=2)  # 压缩到 16x16
        self.layer3 = self._make_layer_(128, 256, num=6, stride=2) # 压缩到 8x8
        self.layer4 = self._make_layer_(256, 512, num=3, stride=2) # 压缩到 4x4

        self.pool2 = nn.AdaptiveAvgPool2d((1,1))#256*1*1
